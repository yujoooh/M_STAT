...
do
  read(1,*,end=10) co
  open(2,file='../../01_관측자료/Output/'//trim(co)//'.dat',status='old')
  open(3,file='../../02_모델자료/Output/'//trim(co)//'.dat',status='old')
  open(4,file='그림/'//trim(co)//'.dat')
  do i=1,9 ; read(2,*) ; enddo
  do i=1,1 ; read(3,*) ; enddo
  do
    read(2,*,end=11) dum0,dum1,d0,d0,d1
    do
      read(3,*,end=12) dum2,d0,d0,d2
